...
export default Cleanliness = ({ navigation, route }) => {
  const [selected, setselected] = useState();
  const language = useContext(LanguageContext);
  const [error, setError] = useState();

  const data = { ...route.params, cleanliness: selected };
  ...
  const nextHandler = () => {
    if (selected === undefined) {
      ...
      navigation.navigate("Suggestions", data);
    }
  };
  return (
    <RatingScreen
      text={language.english ? "Cleanliness" : "النظافه"}
