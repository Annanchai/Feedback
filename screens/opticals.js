...
export default Opticals = ({ navigation, route }) => {
  const [selected, setselected] = useState();
  const language = useContext(LanguageContext);
  const [error, setError] = useState();

  const data = { ...route.params, opticals: selected };
  ...
  const nextHandler = () => {
    if (selected === undefined) {
      ...
      navigation.navigate("Cleanliness", data);
    }
  };
  return (
    <RatingScreen
      text={language.english ? "Opticals" : "النظارات"}
